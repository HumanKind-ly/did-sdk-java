...
/**
 * A simple, manually constructed example of a membership verifiable credential document.
 */
public class MembershipDocument extends HcsVcDocumentBase<Membership> {
  private static final String DOCUMENT_TYPE = "Membership";
  private static final String EXAMPLE_ID_PREFIX = "http://localhost:5050/membership/";
  ...
  public String toNormalizedJson(final boolean withoutProof) {
    ...
    for (String property : JSON_PROPERTIES_ORDER) {
      if (JSON_PROPERTY_CREDENTIAL_SUBJECT.equals(property) && getCredentialSubject() != null) {
        for (Membership dl : getCredentialSubject()) {
