...
import com.hedera.hashgraph.identity.hcs.example.appnet.dto.MembershipRequest;
...
import com.hedera.hashgraph.identity.hcs.example.appnet.vc.Membership;
import com.hedera.hashgraph.identity.hcs.example.appnet.vc.MembershipDocument;
 ...
 */
public class MembershipHandler extends AppnetHandler {
  private static Logger log = LoggerFactory.getLogger(MembershipHandler.class);
   ...
   */
  public MembershipHandler(final HcsIdentityNetwork identityNetwork, final AppnetStorage storage) {
  ...
  /**
   * Generates a new example verifiable credential document - a membership.
   ...
   * @param ctx The HTTP context.
   */
  public void generateMembership(final Context ctx) {
    ctx.getRequest().getBody().then(data -> {
      MembershipRequest req = null;
      try {
        req = JsonUtils.getGson().fromJson(data.getText(), MembershipRequest.class);
      } catch (Exception e) {
        ctx.getResponse().status(Status.BAD_REQUEST);
      ...
      log.info("Generating new membership document for: " + req.getOwner());

      try {
        MembershipDocument vc = new MembershipDocument();
        ...
        ctx.render(Jackson.json(new ErrorResponse("Membership generation failed.", e)));
        return;
      }
    });
  }
  ...
  public void determineCredentialHash(final Context ctx) {
    ctx.getRequest().getBody().then(data -> {
      try {
        String hash = HcsVcDocumentBase.fromJson(data.getText(), Membership.class).toCredentialHash();
        ctx.render(hash);
      } catch (Exception e) {
        ctx.getResponse().status(Status.BAD_REQUEST);
